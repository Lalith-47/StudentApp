...
async function testSystemHealth() {
  ...
  try {
    // Backend health
    const backendHealth = await axios.get(`${BASE_URL.replace('/api', '')}/health`);
...
async function testAuthentication() {
  ...
  try {
    ...
    logTest('Authentication Tests', 'FAIL', error.response?.data?.message || error.message);
  }
...
async function testStudentFlow(token) {
  ...
  try {
    // Student dashboard
    const dashboardResponse = await axios.get(`${BASE_URL}/student-enhanced/dashboard`, { headers });
    ...
    logTest('Student Dashboard Access', 'FAIL', error.response?.data?.message || error.message);
  }
  
  try {
    // Student courses
    const coursesResponse = await axios.get(`${BASE_URL}/student-enhanced/courses`, { headers });
    ...
    logTest('Student Courses Access', 'FAIL', error.response?.data?.message || error.message);
  }
}
...
async function testPublicAPIs() {
  ...
  for (const endpoint of publicEndpoints) {
    try {
      ...
      logTest(endpoint.name, 'FAIL', error.response?.data?.message || error.message);
    }
  }
}
...
async function testDatabaseOperations() {
  ...
  try {
    ...
    logTest('Database Connectivity', 'FAIL', error.response?.data?.message || error.message);
  }
}
...
async function testErrorHandling() {
  ...
  try {
    ...
    if (error.response?.status === 400) {
      ...
      logTest('Invalid Input Handling', 'FAIL', `Unexpected status: ${error.response?.status}`);
    }
  }
}

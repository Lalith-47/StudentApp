...
const getFaqById = async (req, res) => {
  try {
    ...
    if (!faq) {
      faq = dummyFaqs.find(f => f._id === id || f.question.toLowerCase().includes(id.toLowerCase()));
...
const submitFaqQuery = async (req, res) => {
  try {
    ...
    // Try to find matching FAQs in database first
    let matchingFaqs;
    try {
    ...
    if (matchingFaqs.length > 0) {
      ...
      response = {
        found: false,
        answer: "I couldn't find a specific answer to your question. Please try rephrasing your query or contact our support team for assistance.",
        suggestions: [
...
const markFaqHelpful = async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: {
        helpful: faq.helpful,
        helpfulPercentage: faq.helpfulPercentage || Math.round((faq.helpful.yes / (faq.helpful.yes + faq.helpful.no)) * 100)
      }
    });
...
function calculateConfidence(query, faq) {
  ...
  const confidence = Math.min(95, Math.round((matches / queryWords.length) * 100));
  return confidence;
}
